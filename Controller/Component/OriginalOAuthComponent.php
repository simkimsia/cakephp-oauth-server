 ...
 * }}}
 *
 * You can also use OAuthComponent::ALL instead of the string 'all'
 *
 * @var mixed
 ...
 */
	public function startup($controller) {
		...
		try {
			...
			$user = $this->user(null, $this->AccessToken->id);
		...
		return false; // originally return true
	}
 ...
 */
	public function isAuthorized($user = null, $request = null) {
		...
		$this->log('before isauthorized check user');
		$this->log($user);
 ...
 */
	public function constructAuthorize() {
		...
		$global = array();
		if (isset($config[OAuthComponent::ALL])) {
			$global = $config[OAuthComponent::ALL];
			unset($config[OAuthComponent::ALL]);
		}
		foreach ($config as $class => $settings) {
			list($plugin, $class) = pluginSplit($class, true);
			$className = $class . 'Authorize';
			App::uses($className, $plugin . 'Controller/Component/OAuth');
